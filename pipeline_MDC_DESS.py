...
scalar_names= ['Diff_thickness (mm)', 'Diff_T2_mean_filt']
visits = ['VISIT-2']
knees_to_use = ['ctrl']
knee = 'ctrl'
visit = 'VISIT-2'
...
output_file = os.path.join(log_path, f'MDC_controls.txt')
mean_data_path = os.path.join(code_dir_path, 'mean_data')
save_path = os.path.join(mean_data_path, 'MDC_controls')
mean_femur_path = os.path.join(mean_data_path, 'MeanFemur_mesh_B-only.vtk')
mdc_femur_path =os.path.join(save_path, f'MeanFemur_mesh_B-only_{knee}_{visit}_MDC.vtk')
...
for scalar_name in scalar_names:
    ...
    results = {knee: {visit: {f'{scalar_name}_MDC': None} for visit in visits} for knee in knees_to_use}
    ...
    for knee in knees_to_use:
        for visit in visits:
            ...
            for i in range(scalar_array.shape[1]):
                ...
                bone_ratio_param1 = np.sum(scalar_array[:, i] == -200) / len(scalar_array[:, i])
                fc_edges_ratio_param1 = np.sum(scalar_array[:, i] == -100) / len(scalar_array[:, i])
            
                # Check if more than x% of the values are zeros
                if bone_ratio_param1 > 0.05:
                    MDC[i] = 0.00  # Assign 0.0 for r-values
                    
                elif fc_edges_ratio_param1 > 0.05:
                    MDC[i] = 0.00   # Assign 0.0 for r-values
                    ...
                    # SEM = SD/sqrt(2)
                    MDC[i] = 1.96* (np.std(scalar_array[:, i]))

            # Store results in the dictionary
            results[knee][visit] = {
                f'{scalar_name}_MDC': MDC,
            }
            ...
            femur_mesh.point_data[f'{scalar_name}_MDC'] = results[knee][visit][f'{scalar_name}_MDC']
