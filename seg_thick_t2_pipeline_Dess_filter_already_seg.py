...
def main(path_image, path_save, path_save_t2c, path_config, model_name='acl_qdess_bone_july_2024'):
    ...
    sitk_seg = sitk.ReadImage(os.path.join(path_save, filename_save + '_all-labels.nrrd'))
    # sitk_seg = sitk.Cast(sitk_seg, sitk.uint8)
    ...
    map_path_nrrd_filt= map_path_filt_nii.replace('.nii.gz', '.nrrd')
    map_path_nrrd= os.path.join(path_save, f'{filename_save}_t2map.nrrd')
    
    _ = filter_t2maps(t2_map_path=map_path_nii, fwhm= 1, t2_map_save_path=map_path_filt_nii)
    ...
    mesh_femur, data_probe_t2_filt = project_T2_data(mesh_femur, mesh_fc, map_path_nrrd_filt)
    ...
    #####################################################################################################################
    print('Chopping Femur Shaft to 70% of the S-I direction...')
    ...
    if dimension_z > 0.7 * dimension_y: # if the S-I dimension is greater than 70% of the R-L dimension
        ...
        z_min = bounds[4]   
        z_max = bounds[4] + 0.6 * abs(bounds[1] - bounds[0])  # Define your maximum x value
    ...
    else: # if the S-I dimension is less than 70% then just crop to 95% of the current S-I height to open up the top surface
        # Define the cropping parameters
        z_min = bounds[4]   
