...
def main(config_path, path_image, path_save, path_seg_script, log_file_path):
    # Load configuration with default model name
    with open(config_path) as f:
        config = json.load(f)
    
    model_name = sys.argv[4] if len(sys.argv) > 4 else config.get('default_seg_model', 'acl_qdess_bone_july_2024')
    ...
    if run_subprocess(seg_command, "Segmentation Pipeline") != 0:
        with open(log_file_path, 'a') as f:
            f.write("\n Segmentation pipeline failure. Check again \n")
            f.write(path_image)
    ...
    for analysis in nsm_analyses:
        ...
        if run_subprocess(command, analysis["script"]) != 0:
            with open(log_file_path, 'a') as f:
                f.write(f"\n, Warning: {analysis['script']} failed, continuing with other analyses. \n")
                f.write(path_image)
...
if __name__ == "__main__":
    ...
    # get path seg script   
    path_seg_script = os.path.join(current_dir, 'seg_thick_t2_pipeline_Dess_filter_already_seg.py')
    # path_seg_script = os.path.join(current_dir, 't2_filter_pipeline_Dess.py')
    
    main(config_path, path_image, path_save, path_seg_script, log_file_path)