...
output_file = os.path.join(log_path, f'shape_change_overtime_SD.txt')
...
for model_type in model_types:
    ...
    save_path = os.path.join(save_dir_path, f'{model_type_names[model_type]}_model/shape_change_overtime_SD')
    ...
    # Save to npz file
    file_save_path = os.path.join(save_path, f'latents_all.npz')
    if not os.path.exists(file_save_path):
        np.savez(file_save_path, subjects=subjects, visits=visits, knees=knees, latents=latents)
                                
    ################################ Creating mean latent vectors for each knee and SD of chnage overtime ################################
    ...
    ###########################################Create STDEV of chnage #######################################################
  
    for knee in knees_to_use:
    
        # Load the mean latent vectors for each knee and visit
        mean_latents = np.load(os.path.join(save_path, f'mean_latent_{knee}.npz'))

        # Define the visits you are interested in
        visits = range(1, 6)  # Assuming visits are numbered 1 to 5
        std_intervals = np.arange(0, 3.5, 0.5)  # Use 3.5 to ensure 3 is included

        mean_diff = mean_latents['Visit-5'] - mean_latents['Visit-1']

        # Initialize dictionaries to hold the new latent vectors
        latent_data = {}
        
        # Iterating through each visit
        for std_interval in std_intervals:
            
            # Calculate the new latent vector
            new_latent = mean_latents[f'Visit-1'] + std_interval * mean_diff

            # Store the new latent vectors in the dictionaries
            latent_data[f'{std_interval}SD'] = new_latent

        # Save the new latent vectors to .npz files for each knee
        latent_file_save_path = os.path.join(save_path, 'mean_latent_change{knee}.npz')
        np.savez(latent_file_save_path, **latent_data)
    ...
    for knee in knees_to_use:
        mean_save_path = os.path.join(save_path, f'{knee}') 
        if not os.path.exists(mean_save_path):
            os.makedirs(mean_save_path)
            mesh_save_path = os.path.join(save_path, f'{knee}')
            file_save_path = os.path.join(mesh_save_path, f'{knee}.npz')
            
            for std_interval in std_intervals:
                
                if std_interval == 0.0:

                    print(f'Creating reference mean mesh for {knee} at {std_interval}SD')
                    latent_file_save_path =os.path.join(save_path, 'mean_latent_change{knee}.npz')
                    latent= np.load(latent_file_save_path)               
                    latent_vector = latent[f'{std_interval}SD'].squeeze()
            
                    if model_type == 'bone_only':
                        bone_mesh= create_mean_mesh(latent_vector, model, model_type)
                        ref_mesh_path_femur =os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{knee}_{std_interval}SD.vtk')
                        bone_mesh.save_mesh(ref_mesh_path_femur)
        
                    elif model_type == 'bone_cartilage':
                        bone_mesh, cart_mesh= create_mean_mesh(latent_vector, model, model_type)
                        ref_mesh_path_femur =os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{knee}_{std_interval}SD.vtk')
                        ref_mesh_path_cart = os.path.join(mesh_save_path, f'MeanCart_NSMrecon_{knee}_{std_interval}SD.vtk')
                        bone_mesh.save_mesh(ref_mesh_path_femur)
                    ...
                    if model_type == 'bone_only':
                        ...
                        print(f'Computing deformation from 0 SD to {std_interval} SD')
                        ref_latent_vector = latent[f'0.0SD'].squeeze()
                        org_mesh_path_femur = os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{knee}_0.0SD.vtk')

                        interp_mesh_femur= interpolate_matching_points(ref_latent_vector, [org_mesh_path_femur], latent_vector, model, model_type)
                        interp_mesh_path_femur =os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{knee}_{std_interval}SD.vtk')    
                        interp_mesh_femur.save_mesh(interp_mesh_path_femur)
                        org_mesh_path_femur =os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{knee}_{std_interval}SD.vtk')
                        ...
                        ref_latent_vector = latent[f'0.0SD'].squeeze()
                        org_mesh_path_femur= os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{knee}_0.0SD.vtk')
                        org_mesh_path_cart = os.path.join(mesh_save_path, f'MeanCart_NSMrecon_{knee}_0.0SD.vtk')
