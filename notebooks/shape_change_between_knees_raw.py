...
output_file = os.path.join(log_path, f'shape_change_between_knees_raw.txt')
...
try:
    for model_type in model_types:
        ...
        save_path = os.path.join(save_dir_path, f'{model_type_names[model_type]}_model/shape_change_between_knees_raw')
        ...
        for knee_combination in knee_combinations:
            ...
            print(f'Processing change from {ref_knee} to {target_knee}')
        ...
        #     ref_latent = latent[ref_knee].squeeze()
        #     target_latent = latent[target_knee].squeeze()
            
            ref_femur_mesh_path = os.path.join(save_path, f'MeanFemur_NSMrecon_{ref_knee}.vtk')
            ref_cart_mesh_path = os.path.join(save_path, f'MeanCart_NSMrecon_{ref_knee}.vtk')
            
            if model_type == 'bone_only':
                pass
        #         interp_mesh= interpolate_matching_points(ref_latent, [ref_femur_mesh_path], target_latent, model, model_type)
        #         interp_mesh_path =os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{ref_knee}_to_{target_knee}.vtk')
        #         interp_mesh.save_mesh(interp_mesh_path)
        #         print(f'Computing deformation from Visit-1 to {visit}')
        #         deformed_mesh = compute_signed_distance_and_normal(ref_femur_mesh_path, interp_mesh_path)
        #         deformed_mesh.save_mesh(interp_mesh_path)
            
            elif model_type == 'bone_cartilage':
        #         interp_mesh_femur, interp_mesh_cart = interpolate_matching_points(ref_latent, [ref_femur_mesh_path, ref_cart_mesh_path], target_latent, model, model_type)
                
                interp_mesh_path_femur =os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{ref_knee}_to_{target_knee}.vtk')    
                interp_mesh_path_cart =os.path.join(mesh_save_path, f'MeanCart_NSMrecon_{ref_knee}_to_{target_knee}.vtk')
        ...
        #         print(f'Computing deformation from Visit-1 to {visit}')
        #         deformed_mesh_femur = compute_signed_distance_and_normal(ref_femur_mesh_path, interp_mesh_path_femur)
        #         deformed_mesh_cart = compute_signed_distance_and_normal(ref_cart_mesh_path, interp_mesh_path_cart)
        #         deformed_mesh_femur.save_mesh(interp_mesh_path_femur)
        #         deformed_mesh_cart.save_mesh(interp_mesh_path_cart)   
                calculate_fc_thickess_changes(ref_femur_mesh_path, interp_mesh_path_femur)
            else:
                raise ValueError(f'Unknown model type: {model_type}')
