...
try:
    for model_type in model_types:
         for change_type in change_types:
            ...
            save_path = os.path.join(save_dir_path, f'{model_type_names[model_type]}_model/MinMax_shape_subjectwise')
            ...
            for visit, group in data.groupby('Visit'):
                if change_type == 'largest':
                    ...
                    aclr_best = group[group['Knee'] == 'aclr'].nlargest(1, f'{model_type_names[model_type]}Score')
                    clat_best = group[group['Knee'] == 'clat'].nlargest(1, f'{model_type_names[model_type]}Score')
                    ctrl_best = group[group['Knee'] == 'ctrl'].nlargest(1, f'{model_type_names[model_type]}Score')
                else:
                    # Find the bottom for BCScore Diff
                    aclr_best = group[group['Knee'] == 'aclr'].nsmallest(1, f'{model_type_names[model_type]}Score')
                    clat_best = group[group['Knee'] == 'clat'].nsmallest(1, f'{model_type_names[model_type]}Score')
                    ctrl_best = group[group['Knee'] == 'ctrl'].nsmallest(1, f'{model_type_names[model_type]}Score')
