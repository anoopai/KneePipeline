...
output_file = os.path.join(log_path, f'shape_change_overtime_raw_wrt_previous_timepoint.txt')
...
try:
    for model_type in model_types:
        ...
        save_path = os.path.join(save_dir_path, f'{model_type_names[model_type]}_model/shape_change_overtime_raw_wrt_previous_timepoint')
        ...
        ################################ Creating mean latent vectors for each knee and visit ################################
        ...
        for knee in knees_to_use:
            ...
            for visit in latent.files:
                
                ref_latent_vector = latent[visit].squeeze()
                print(f'Creating reference mean mesh for {knee} knee, Visit-{visit}')
                
                if model_type == 'bone_only':
                    bone_mesh= create_mean_mesh(ref_latent_vector, model, model_type)
                    ref_mesh_path_femur =os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{knee}_{visit}.vtk')
                    ...
                    ref_mesh_path_femur =os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{knee}_{visit}.vtk')
                    ref_mesh_path_cart =os.path.join(mesh_save_path, f'MeanCart_NSMrecon_{knee}_{visit}.vtk')
                ...
                if visit != 'Visit-1':
                    ...
                    previous_visit = f'Visit-{int(visit[-1]) - 1}'
                    print(f'Creating mean mesh for {knee} {visit} by interpolating points from Visit-1 and computing deformation')
                    ref_latent = latent[previous_visit].squeeze()
                    target_latent = latent[visit].squeeze()
                    
                    if model_type == 'bone_only':
                        ref_mesh_path_femur =os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{knee}_{previous_visit}.vtk')
                        interp_mesh= interpolate_matching_points(ref_latent, [ref_mesh_path_femur], target_latent, model, model_type)
                        interp_mesh_path =os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{knee}_{previous_visit}_to_{visit}.vtk')
                        interp_mesh.save_mesh(interp_mesh_path)
                        print(f'Computing deformation for {knee} knee from {previous_visit} to {visit}')
                        ...
                        interp_mesh_path_femur =os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{knee}_{previous_visit}_to_{visit}.vtk')    
                        interp_mesh_path_cart =os.path.join(mesh_save_path, f'MeanCart_NSMrecon_{knee}_{previous_visit}_to_{visit}.vtk')
                        ...
                        print(f'Computing deformation for {knee} knee from {previous_visit} to {visit}')
                        ref_mesh_path_femur =os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{knee}_{previous_visit}.vtk')
                        ref_mesh_path_cart =os.path.join(mesh_save_path, f'MeanCart_NSMrecon_{knee}_{previous_visit}.vtk')
