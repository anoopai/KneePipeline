import os
from pymskt.mesh import get_icp_transform, cpd_register
...
save_path = '/dataNAS/people/anoopai/DESS_ACL_study/results/BScore_and_FC_metrics/mean_shape_recon'
...
if n_objects == 1:
    ...
    bone_mesh.save_mesh(os.path.join(save_path, f'BO_MeanFemur_mesh.vtk'))
elif n_objects == 2:
    bone_mesh, cart_mesh = create_mesh(decoder=model, latent_vector=latent_vector, objects=n_objects)
    bone_mesh.save_mesh(os.path.join(save_path, f'BC_MeanFemur_mesh.vtk'))
    cart_mesh.save_mesh(os.path.join(save_path, f'BC_MeanCart_mesh.vtk'))
