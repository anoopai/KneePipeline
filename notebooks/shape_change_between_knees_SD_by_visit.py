...
output_file = os.path.join(log_path, f'shape_change_between_knees_SD_by_visit.txt')
...
try:
    for model_type in model_types:
        ...
        save_path = os.path.join(save_dir_path, f'{model_type_names[model_type]}_model/shape_change_between_knees_SD_by_visit')
        ...
        ##################### Compute the mean shape difference between the two knee-groups ##########################
        visits = range(1, 6)
        ...
        for knee_combination in knee_combinations:
            ...
            for visit in visits:
                ...
                target_latent= np.load(target_latent_path)
                
                ref_latent_vector = ref_latent[f'Visit-{visit}']
                target_latent_vector = target_latent[f'Visit-{visit}']
                
                # Calculate the mean shape difference
                mean_latent_diff = target_latent_vector - ref_latent_vector

                # create latents along the clat to aclr vector at specific intervals
                std_intervals = np.arange(0, 3.5, 0.5)  # Use 3.5 to ensure 3 is included

                # Dictionary to hold all new latent vector variations
                latent_data_knee = {}
                
                for std_interval in std_intervals:
                    
                    # Calculate the new latent vector
                    new_latent_change = ref_latent_vector + std_interval * mean_latent_diff
                            
                    # Save the new latent vectors in the dictionary
                    latent_data_knee[f'{std_interval}SD'] = new_latent_change
                    
                # Save the new latent vectors to .npz files
                file_save_path = os.path.join(mesh_save_path, f'{ref_knee}_to_{target_knee}_Visit-{visit}.npz')
                np.savez(file_save_path, **latent_data_knee)
                    
        #############################  Create mean meshes from NSM model and latents ##############################   
        ...
        for knee_combination in knee_combinations:
            ...
            for visit in visits:    
                ...
                # Load the latent vectors for the current knee combination and visit
                ref_knee= knee_combination[0]
                target_knee= knee_combination[1]
                ...
                for std_interval in std_intervals:
                    ...
                    print(f'Creating reference mean mesh for {ref_knee} to {target_knee} for Visit-{visit} at {std_interval}SD')
                    latent= np.load(file_save_path)               
                    latent_vector = latent[f'{std_interval}SD'].squeeze()
                
                    if model_type == 'bone_only':
                        bone_mesh= create_mean_mesh(latent_vector, model, model_type)
                        ref_mesh_path_femur =os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{ref_knee}_to_{target_knee}_Visit-{visit}_{std_interval}SD.vtk')
                        bone_mesh.save_mesh(ref_mesh_path_femur)
                        
                        print(f'Computing deformation from {ref_knee} to {target_knee} for Visit {visit} at {std_interval}SD')
                        ref_latent_vector = latent[f'0.0SD'].squeeze()
                        org_mesh_path_femur = os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{ref_knee}_to_{target_knee}_Visit-{visit}_0.0SD.vtk')
                        interp_mesh_femur = interpolate_matching_points(ref_latent_vector, [org_mesh_path_femur], latent_vector, model, model_type)
                        interp_mesh_femur.save_mesh(ref_mesh_path_femur)
                        deformed_mesh_femur = compute_signed_distance_and_normal(org_mesh_path_femur, ref_mesh_path_femur)
                        deformed_mesh_femur.save_mesh(ref_mesh_path_femur)
                    
                    elif model_type == 'bone_cartilage':
                        bone_mesh, cart_mesh= create_mean_mesh(latent_vector, model, model_type)
                        ref_mesh_path_femur =os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{ref_knee}_to_{target_knee}_Visit-{visit}_{std_interval}SD.vtk')
                        ref_mesh_path_cart = os.path.join(mesh_save_path, f'MeanCart_NSMrecon_{ref_knee}_to_{target_knee}_Visit-{visit}_{std_interval}SD.vtk')
                        bone_mesh.save_mesh(ref_mesh_path_femur)
                        ...
                        ref_latent_vector = latent[f'0.0SD'].squeeze()
                        org_mesh_path_femur = os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{ref_knee}_to_{target_knee}_Visit-{visit}_0.0SD.vtk')
                        org_mesh_path_cart = os.path.join(mesh_save_path, f'MeanCart_NSMrecon_{ref_knee}_to_{target_knee}_Visit-{visit}_0.0SD.vtk')
                        interp_mesh_femur, interp_mesh_cart = interpolate_matching_points(ref_latent_vector, [org_mesh_path_femur, org_mesh_path_cart], latent_vector, model, model_type)
                        interp_mesh_femur.save_mesh(ref_mesh_path_femur)
                        interp_mesh_cart.save_mesh(ref_mesh_path_cart)
                        project_fc_thickness_on_femur_mesh(ref_mesh_path_femur, ref_mesh_path_cart)
                        
                        # Compute the signed distance and normal vectors for the deformed meshes
                        deformed_mesh_femur = compute_signed_distance_and_normal(org_mesh_path_femur, ref_mesh_path_femur)
                        deformed_mesh_cart = compute_signed_distance_and_normal(org_mesh_path_cart, ref_mesh_path_cart)
                        deformed_mesh_femur.save_mesh(ref_mesh_path_femur)
                        deformed_mesh_cart.save_mesh(ref_mesh_path_cart) 
                        calculate_fc_thickess_changes(org_mesh_path_femur, ref_mesh_path_femur)           
                    else:
                        raise ValueError(f'Unknown model type: {model_type}')
