...
output_file = os.path.join(log_path, f'shape_change_overtime_raw.txt')
...
try:
    for model_type in model_types:
        ...
        save_path = os.path.join(save_dir_path, f'{model_type_names[model_type]}_model_trial/shape_change_overtime_raw_shaft_crop')
            ...
            for visit in latent.files:
                ...
                if visit == 'Visit-1':
                    ...
                    if model_type == 'bone_only':
                        ...
                        interp_mesh_path =os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{knee}_Visit-1_to_{visit}.vtk')
                        ...
                        mesh_org = pv.read(ref_mesh_path_femur)
                        ...
                        ref_mesh_path_femur = os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{knee}_Visit-1_clipped.vtk')
                        io.write_vtk(clipped_mesh, ref_mesh_path_femur, points_dtype=float, write_binary=False)
                        
                        bone_mehs = BoneMesh(ref_mesh_path_femur)
