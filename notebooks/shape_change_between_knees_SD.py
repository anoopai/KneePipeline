...
output_file = os.path.join(log_path, f'shape_change_between_knees_SD.txt')
...
try:
    for model_type in model_types:
        ...
        save_path = os.path.join(save_dir_path, f'{model_type_names[model_type]}_model/shape_change_between_knees_SD')
        ...
        # # # Calculate the mean latent vector for each knee
        # for knee in unique_knees:
            
        #         knee_str = knee.decode('utf-8') if isinstance(knee, bytes) else str(knee)
        #         indices = np.where(knees == knee)[0]
        #         mean_latents[knee_str] = np.mean(latents[indices], axis=0)
        
        # file_save_path = os.path.join(save_path, f'mean_latent.npz')
        # np.savez(file_save_path, **mean_latents)
        ...
        #         knee_latent_path = os.path.join(save_path, f'mean_latent.npz')
        #         knee_latent= np.load(knee_latent_path)
                
        #         ref_latent_vector = knee_latent[ref_knee]
        #         target_latent_vector = knee_latent[target_knee]
        ...
        #         file_save_path = os.path.join(mesh_save_path, f'{ref_knee}_to_{target_knee}.npz')
        ...
        for knee_combination in knee_combinations:
            ...
            for visit in visits:    
                ...
                for std_interval in std_intervals:
                    ...
                    print(f'Creating reference mean mesh for {ref_knee} to {target_knee} at {std_interval}SD')
                    ...
                    if model_type == 'bone_only':
                        ...
                        # ref_mesh_path_femur =os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{ref_knee}_to_{target_knee}_{std_interval}SD.vtk')
                        ...
                        # ref_latent = latent[f'0.0SD'].squeeze()
                        # org_mesh_path_femur = os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{ref_knee}_to_{target_knee}_0.0SD.vtk')
                        # interp_mesh_femur = interpolate_matching_points(ref_latent, [org_mesh_path_femur], latent_vector, model, model_type)
                        ...
                        ref_mesh_path_femur =os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{ref_knee}_to_{target_knee}_{std_interval}SD.vtk')
                        ref_mesh_path_cart = os.path.join(mesh_save_path, f'MeanCart_NSMrecon_{ref_knee}_to_{target_knee}_{std_interval}SD.vtk')
                        # bone_mesh.save_mesh(ref_mesh_path_femur)
                        ...
                        # ref_latent = latent[f'0.0SD'].squeeze()
                        org_mesh_path_femur = os.path.join(mesh_save_path, f'MeanFemur_NSMrecon_{ref_knee}_to_{target_knee}_0.0SD.vtk')
                        org_mesh_path_cart = os.path.join(mesh_save_path, f'MeanCart_NSMrecon_{ref_knee}_to_{target_knee}_0.0SD.vtk')
                        # interp_mesh_femur, interp_mesh_cart = interpolate_matching_points(ref_latent, [org_mesh_path_femur, org_mesh_path_cart], latent_vector, model, model_type)
                        # interp_mesh_femur.save_mesh(ref_mesh_path_femur)
