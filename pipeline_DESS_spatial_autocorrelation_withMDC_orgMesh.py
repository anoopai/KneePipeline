...
log_file_path = os.path.join(log_path, f'pipeline_DESS_spatial_autocorrelation_withMDC_orgMesh.txt')
mean_path = os.path.join(code_dir_path, 'mean_data')
save_path = os.path.join(mean_path, f'spatial_autocorrelation/orgMesh')
mean_femur_path = os.path.join(mean_path, 'MeanFemur_mesh_B-only.vtk')
mdc_femur_path = os.path.join(save_path, 'MDC_Mean_orgMesh_femur_mesh_ctrl_VISIT-2.vtk')
...
for k_neighbors in k_neighbors_list:
    for permutation in permutations:
        for knee in knees:
            for visit in visits:                
                vtk_file = os.path.join(save_path, f'Mean_orgMesh_femur_mesh_{knee}_{visit}.vtk') # Replace with your VTK file path
                ...
                for scalar_name in scalar_names:
                    ...
                    unique_points = np.unique(points_filt, axis=0)

                    if points_filt.shape[0] > k_neighbors and unique_points.shape[0] > k_neighbors:
