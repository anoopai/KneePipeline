...
def main(path_image, path_save, path_config, model_name='acl_qdess_bone_july_2024'):
    ...
    if (qdess is not None):
        ...
        if include_required_tags:
            ...
            # project T2 on to the femur bone surface
            # t2_nrrd_path= os.path.join(path_save, f'{filename_save}_t2map.nrrd')
            # bone_mesh.calc_cartilage_t2(path_t2_nrrd=t2_nrrd_path, 
            # bone_mesh.save_mesh(os.path.join(path_save, f'{bone_name}_mesh.vtk'))
            # dict_bones[bone_name]['mesh'] = bone_mesh
            # thickness =  np.array(bone_mesh.get_scalar("thickness (mm)"))
            # t2 = np.array(bone_mesh.get_scalar("t2 (ms)"))
    ...
    #####################################################################################################################
    mesh_org = pv.read(os.path.join(path_save, f'femur_mesh.vtk') ) # Replace with the path to your .vtk file
    ...
    io.write_vtk(clipped_mesh, file_path, scalar_name=None, points_dtype=float, write_binary=False)
